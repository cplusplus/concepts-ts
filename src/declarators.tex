...
\terminal{D1} \terminal{(} parameter-declaration-clause \terminal{)} cv-qualifier-seq\opt\br
  \hspace*{\bnfindentinc}ref-qualifier\opt 
...
equivalent predicate constraints (\ref{temp.param}), using the rules for 
comparing constraints in \ref{temp.constr.constr}, have the same invented 
template parameter.
% 
\enterexample
\begin{codeblock}
...
\tcode{U} is the template parameter invented for \tcode{tail} (\ref{temp.param}).
\end{addedblock}
\end{quote}
