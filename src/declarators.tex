...
parameter matches the kind (type, non-type, template) of the prototype parameter 
(\ref{dcl.spec.concept}) of the concept designated by the 
\grammarterm{constrained-type-specifier} (\ref{temp.constr.resolve}).
\end{itemize}
% 
The invented template parameter may be required or adjusted to declare a
template parameter pack:
\begin{itemize}
\item if the invented template parameter corresponds to a 
\grammarterm{constrained-type-specifier}, and the prototype parameter of its 
designated concept declares a template parameter pack, the 
\grammarterm{constrained-type-specifier} shall be followed by a
\grammarterm{declarator-id} or \grammarterm{abstract-declarator} beginning with 
an ellipsis;

\item otherwise, if the invented \grammarterm{template-parameter} is a parameter 
pack if the \grammarterm{simple-type-specifier} designating the placeholder 
is followed by a \grammarterm{declarator-id} or \grammarterm{abstract-declarator}
that begins with an ellipsis.
\end{itemize}
% 
...
void g5(C4...);
void g6(Tuple<C4...>);
void g7(C4 p);      // error: \tcode{p} does not declare a function parameter pack
void g8(Tuple<C4>); // error: \tcode{C4} is not a pack expansion
...
template<C4... Ts> void g5(Tuple<Ts...>);   // redeclaration of \tcode{g6}
\end{codeblock}
\exitexample
