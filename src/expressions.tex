...
template<typenamem F, typename... Args>
  concept bool C1() { 
    requires(F f, Args... args) { f(args...); }; // OK: declares function parameter pack
  }
\end{codeblock}
\exitexample
...
\begin{itemize}
\item if \tcode{T} contains no placeholders (\ref{dcl.spec.auto}, 
\ref{dcl.spec.constr}), the requirement appends two constraints:
...
\item otherwise, \tcode{T} contains one or more placeholders, and the
requirement appends a deduction constraint (\ref{temp.constr.atom.deduct}) of 
\tcode{E} against the type \tcode{T}.
\end{itemize}
\end{itemize}
%
\enterexample
