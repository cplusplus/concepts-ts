...
% TODO: Move this into 14.2
\pnum
A template declared by a \grammarterm{template-introduction}
...
by the \grammarterm{template-introduction}.
...
\pnum
\begin{bnf}
\nontermdef{template-parameter}
  \added{constrained-parameter}

\added{
\nontermdef{constrained-parameter}\br
  constrained-type-specifier ...\opt identifier\opt\br
  constrained-type-specifier ...\opt identifier\opt \terminal{=} type-id\br
  constrained-type-specifier ...\opt identifier\opt \terminal{=} id-expression\br
  constrained-type-specifier ...\opt identifier\opt \terminal{=} initializer-clause\br
}
\end{bnf}
\end{quote}
...
declaration of a \grammarterm{constrained-parameter} and 
...
by its \grammarterm{constrained-type-specifier}.
% 
...
template<C5... Cs> f6();      // OK: \tcode{Cs} is a template parameter pack of chars
\end{codeblock}
\exitexample

\pnum
A \grammarterm{constrained-parameter} associates a
\grammarterm{constraint-expression} with its
...
of default template argument for \grammarterm{constrained-parameter}.
...
\pnum
A \defn{template introduction} provides a more concise way of declaring
...
\pnum
A \grammarterm{template-introduction} associates a 
\grammarterm{constraint-expression} with its
\grammarterm{template-declaration}.
...
parameter pack, its corresponding \grammarterm{template-argument} is a pack
expansion of \tcode{T}. 
% 
% FIXME: Replace type and form
% 
Otherwise, its corresponding \grammarterm{template-argument} matches the 
type and form of \tcode{T}.
% 
...
\grammarterm{template-id} are non-dependent \ref{temp.dep.temp}, the template
...
of \tcode{P} and \tcode{Q} are equivalent (\ref{temp.type}).</p>
...
of \tcode{P} and \tcode{Q} are equivalent (\ref{temp.type}). 
...
the conjunction of constraints (\ref{temp.constr.op.conj}) 
...
\pnum
The formed constraint is the fold expression \tcode{(E \&\& ...)}
(\ref{expr.prim.fold}) when the prototype parameter \tcode{P} declares a 
template parameter pack and the target template parameter \tcode{X}
does not. 
% 
...
void f1(C1&);     // associates C1<T1> with f1
void f2(C2<int>); // associates C2<T2, int>() with f2
void f3(C1...);   // associates (C1<T1> \&\& ...) with f3
void f4(C3...);   // associates C3<T1...> with f4
\end{codeblock}
% 
