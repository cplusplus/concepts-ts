...
template<typename... Ts> struct make_tup(Ts... args) { return Tuple<Ts...>{}; }
...
\pnum
The constraint introduced by a \grammarterm{constrained-type-specifier}
is formed according to the rules in \ref{temp.constr.form}. 
% 
When a \grammarterm{constrained-type-specifier} appears in the
\begin{itemize}
\item type of a \grammarterm{parameter-declaration} of a 
\grammarterm{lambda-expression} (\ref{expr.prim.lambda}), the introduced 
...
\item \grammarterm{trailing-return-type} of a \grammarterm{compound-requirement},
