...
%
\added{\enternote
If a function declarator has the form \tcode{auto D -> T}, where
\tcode{D -> T} is a declarator with a \grammarterm{trailing-return-type} 
\tcode{D} and \tcode{T} contains placeholders, the initial \tcode{auto} does 
...
\rSec4[dcl.spec.auto.deduct]{\tcode{auto} deduction}
...
%%
\rSec4[dcl.spec.constr]{Constrained type specifiers}

Add this section to \ref{dcl.spec.auto}.

\begin{quote}
A \grammarterm{constrained-type-specifier} designates a placeholder
(type, non-type, or template) and introduces an associated constraint that 
must be satisfied by that placeholder's eventual replacement.
...
\pnum
When a \grammarterm{constrained-type-specifier} appears in the type of a 
\grammarterm{parameter-declaration} of a \grammarterm{lambda-expression}
(\ref{expr.prim.lambda}) or function declaration (signifying a generic lambda
or abbreviated function template, respectively), the associated constraint is 
formed according to the rules in \ref{dcl.fct}.
%
\enternote
In a generic lambda, the member function template call operator of the
closure type is an abbreviated function template.
\exitnote
% 
When a \grammarterm{constrained-type-specifier} appears in the 
\grammarterm{trailing-return-type} of a \grammarterm{compound-requirement},
(\ref{expr.prim.req.compound}), the associated constraint is formed according
to the rules in \ref{temp.constr.atom.deduct}.
\end{quote}
