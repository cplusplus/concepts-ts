...
\begin{addedblock}
\pnum
A \grammarterm{constrained-type-specifier} within the declared return 
type of an abbreviated function template does not designate a placeholder 
if its invented template \grammarterm{constrained-parameter} would associate
a predicate constraint (\ref{temp.param}) that is equivalent 
(\ref{temp.constr.constr}) to that of an invented template 
\grammarterm{constrained-parameter} \tcode{P} corresponding to a
placeholder in the \grammarterm{parameter-declaration-clause}. 
% 
That \grammarterm{constrained-type-specifier} is replaced by \tcode{P}.
\enterexample
\begin{codeblock}
...
\end{codeblock}
In the declaration \tcode{f1}, the associated constraints of the invented
template parameters corresponding to the \grammarterm{constrained-type-specifier}{}s
are equivalent; they would both associate \tcode{C<T>}, for an invented template 
parameter \tcode{T}.
% 
In \tcode{f2}, the use of \tcode{C} in the return type would associate the
constraint \tcode{(C<T> \&\& ...)}, which is distinct from the constraint
\tcode{C<T>} associated through the \grammarterm{constrained-type-specifier} 
in the \grammarterm{parameter-declaration-clause}.
\exitexample
\end{addedblock}
\end{quote}
...
\enternote
Equivalent \grammarterm{constrained-type-specifier}{s} in the variable or
return type correspond to the same invented template parameter.
\exitnote
\end{itemize}
\end{addedblock}
% 
...
\pnum
\enternote
The constraint introduced by a \grammarterm{constrained-type-name} 
