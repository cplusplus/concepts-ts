...
\grammarterm{constraint-expression} introduced by those placeholders.
The order of operands in that expression is same as the order in which 
the placeholders appear in the declared type. If the normalized constraint 
for that expression is not satisfied by the values deduced for the
declared type (\ref{temp.constr.decl}), the declaration is ill-formed.
% 
