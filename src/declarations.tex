...
argument deduction.
% 
Obtain \added{a type} \tcode{P} from
\tcode{T} by replacing 
\removed{the occurrences of \tcode{auto}}
\added{each occurrence of a placeholder}
with either a new invented type template parameter \tcode{U}$_I$
\added{according to the rules in \ref{dcl.fct}}
or, if the initialization is copy-list-initialization, with
\tcode{std::initializer_list<U\added{$_I$}>}.
%
Deduce a value for \added{each} \tcode{U}\added{$_I$} using the rules
of template argument deduction from a function call~(\cxxref{temp.deduct.call}),
where \tcode{P} is a
...
substituting the deduced \added{values} of each \tcode{U}\added{$_I$} into 
\tcode{P}.
