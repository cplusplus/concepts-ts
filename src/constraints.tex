...
% A conjunction of constraints is satisfied if and only 
...
\pnum
An \defn{atomic constraint} is formed from a
...
template parameters of the constrained entity, called the \defn{parameter
mapping} (\ref{temp.constr.decl}).
\enternote
...
the normal form of
a logical AND expression (\cxxref{expr.log.and})
...
% 
Normalization transforms a \grammarterm{constraint-expression} into a sequence of 
...
is not a valid atomic constraint because it does not have type \tcode{bool}.
\exitexample
