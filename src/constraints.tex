...
requires the normalization of the associate constraints. 
% 
The normalization of a constraint decomposes predicate constraints into 
conjunctions and disjunctions of \defn{atomic constraints}. An atomic constraint
is a predicate constraint that is in normal form (i.e., it cannot be further
normalized).
% 
The \defn{normal form} of a predicate constraint \tcode{P} is defined as 
follows:
% 
\begin{itemize}
\item The normal form of \tcode{(P)} is the normal form of \tcode{P}.

\item The normal form of \tcode{P || Q} is the disjunction (\ref{temp.constr.op}) 
of the normal form of \tcode{P} and the normal form of \tcode{Q}.

\item The normal form of \tcode{P \&\& Q} is the conjunction (\ref{temp.constr.op}) 
of the normal form of \tcode{P} and the normal form of \tcode{Q}.

\item The normal form of a function call of the form
\tcode{C<A$1$, A$2$, ..., A$N$>()},
...
\item The normal form of an \grammarterm{id-expression}
...
(\ref{expr.prim.req}) is the conjunction of constraints (\ref{temp.constr.op})
introduced by the body of that expression.

\item Otherwise, \tcode{E} is an atomic predicate constraint 
