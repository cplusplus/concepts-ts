...
single \grammarterm{constraint-expression} constraint derived 
by first forming the \defn{cumulative constraint expression} from 
the introduced \grammarterm{constraint-expression}{s}, and then
normalizing that expression as described below.

\pnum
The cumulative constraint expression is formed as follows:

\begin{itemize}
\item If there are no introduced \grammarterm{constraint-expression}{s},
the cumulative constraint expression is \tcode{true}.

\item If there is a single introduced \grammarterm{constraint-expression},
that is the cumulative constraint expression.

\item Otherwise, the cumulative constraint expression is a logical 
...
\end{itemize}
% 
The formation of the cumulative constraint expression 
...
\end{codeblock}
The functions \tcode{f1}, \tcode{f2}, and \tcode{f3} have the cumulative
constraint expression \tcode{C<T>}.
% 
\begin{codeblock}
...
\end{codeblock}
The cumulative constraint expressions of \tcode{f4} and \tcode{f5}
are \tcode{C1<T> \&\& C2<T>}.
% 
\begin{codeblock}
...
\end{codeblock}
The cumulative constraint expression of 
\tcode{f6} is \tcode{C1<T> \&\& C2<T>}, and that of 
\tcode{f7} is \tcode{C2<T> \&\& C1<T>}.
\exitexample

\pnum
Determining the satisfaction of a declaration's 
constraints (\ref{temp.constr.constr}), 
and the partial ordering of declarations by those 
constraints (\ref{temp.constr.order}),
requires that their \grammarterm{constraint-expression}{s}
are first \defn{normalized}.
% 
...
is valid and results in \tcode{T::value} with the mapping
\tcode{T}$\mapsto$\tcode{U*},
...
\pnum A declaration's \defn{associated constraints} are the constraints
derived by normalizing its cumulative constraint expression.
% 
\enterexample
\begin{codeblock}
