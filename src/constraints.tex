...
\pnum
An \defn{argument deduction constraint} is a
...
% FIXME:
\pnum
A \defn{universally quantified constraint} is a constraint that declares a sequence
...
constraint associated by the trailing \grammarterm{requires-clause}.
% 
\begin{codeblock}
...
(\ref{temp.constr.pred}) \tcode{N >= 0} subsumes \tcode{N > 0} for some 
