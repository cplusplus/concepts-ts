...
variables in a block (\ref{stmt.block}), 
in namespace scope (\ref{basic.scope.namespace}), and
in a \grammarterm{for-init-statement} (\ref{stmt.for}).
%
\removed{
...
a function call (\ref{temp.deduct.call}), where \tcode{P} is a function 
...
non-deduced context (\ref{temp.deduct.type}).
% 
...
is the one selected by concept resolution (\ref{temp.concept.resolve}).
...
\tcode{constexpr} declaration (\ref{dcl.constexpr}).
