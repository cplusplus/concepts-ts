...
(\ref{expr.typeid}, \ref{expr.sizeof}, \ref{expr.unary.noexcept}
...
function call operator (\ref{over.call})
...
%
\added{
For a generic lambda, the function call operator (\ref{over.call}) 
...
(\ref{basic.lookup}) or by checking properties of types and expressions.
...
(\ref{temop.constr}) introduced by its \grammarterm{requirement}{}s.
...
\item if the the \tcode{noexcept} specifier is present, the 
