...
of the concept designated by its \grammarterm{concept-name}.
% 
...
\item If \tcode{P} declares a template parameter pack, the declared 
...
is derived form of the \grammarterm{constrained-type-name} \tcode{CC} in the
...
\tcode{C} is not a variadic concept, \tcode{E} is adjusted to be the pack 
expansion \tcode{E...}.
\end{itemize}
% 
...
the \tcode{concept-name} \tcode{C} in the \grammarterm{template-introduction} 
and the sequence of \grammarterm{introduced-parameter}{s}.
...
\tcode{E} is the function call C<A1, ..., A$N$>.
\end{itemize}
% 
\tcode{E} is the expression of the associated predicate constraint.
% 
...
associated constraints (\ref{temp.constr.decl})} are functionally equivalent using 
