...
template<typename T> struct X<T*> { }; // OK
template<C T> struct X<T> { }; // OK
\end{codeblock}
Both partial specializations are more specialized than the primary. In the
first case, \tcode{T*} is more specialized than \tcode{T}. In the second
case, the template arguments are equivalent, but the partial specialization 
is more constrained than the primary.
\exitexample
\end{addedblock}
\end{quote}
...
\setcounter{Paras}{5}
\pnum
Two \grammarterm{template-head}{s} are \defn{equivalent} if they
declare sequences of equivalent \grammarterm{template-parameters}
and have \grammarterm{requires-clause}{s} with equivalent
\grammarterm{constraint-expression}{s}.
Two \grammarterm{template-parameter}{s} are \defn{equivalent} if
\begin{itemize}
\item they declare template parameters of the same kind,
\item if either declares a template parameter pack then both do,
...
then both are, and the \grammarterm{qualified-concept-name}{s} are
equivalent.
\end{itemize}
