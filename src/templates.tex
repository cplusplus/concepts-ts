...
template<typename T>
  struct S {
    ...
    void f2() requires C1<T>; // error: constraints refer to \tcode{T}
    void g1<T>(T);            // OK
...
list (\cxxref{temp.deduct})\added{, and the deduced template arguments 
satisfy the constraints of the partial specialization, if any
(\ref{temp.constr})}.
\enterexample
\begin{codeblock}
struct S { void f(); };
...
S2<int> s; // error: substitution failure in definition of \tcode{S2<int>}
\end{codeblock}
\exitexample
\end{quote}    
