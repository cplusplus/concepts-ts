...
\begin{codeblock}
template<typename T> concept C1 = true;
template<typename... Ts> concept C2() { return true; }
...
\enterexample
\begin{codeblock}
template<typename T, int N, typename... Xs> concept C1 = true;
template<template<typename> class X> concept C2 = true;
...
\enterexample
\begin{codeblock}
template<typename A, typename B = bool> concept C() { return true; }
...
\enterexample
\begin{codeblock}
template<typename T, int N = -1> concept P() { return true; }
...
\enterexample
\begin{codeblock}
template<typename T, typename U> concept C1 = true;
template<typename T, typename U> concept C2() { return true; }
template<typename... Ts> concept C3 = true;
...
default arguments\added{, associated constraints (\ref{temp.constr.decl}),} and 
\grammarterm{noexcept-specifier}{s} of function templates and 
default arguments\added{, associated constraints,} and 
\grammarterm{noexcept-specifier}{s} of member functions of class templates 
are considered definitions;
each default argument\added{, associated constraint,} or 
...
void fn() {
  g(0); // OK: \tcode{C<int>} is satisfied
  g((void*)0); // error: cannot call \tcode{g}, \tcode{C<void*>} is not satisfied
}
\end{codeblock}
\exitexample
...
\pnum
When an \grammarterm{id-expression}
refers to a specialization of a concept, the result of the expression is
the prvalue of type \tcode{bool}. 
%
