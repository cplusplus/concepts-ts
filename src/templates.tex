...
to be a concept. The name of the concept is a as \grammarterm{concept-name}.
\enterexample
\begin{codeblock}
template<typename T>
...
A program that explicitly instantiates or specializes a concept is ill-formed.

\pnum
When an \grammarterm{id-expression}
refers to a specialization of the concept, the result of the expression is
the prvalue of type \tcode{bool}. The expression is \tcode{true} if concept's 
\emph{constraint-expression} is satisfied \ref{temp.constr.constr} and 
